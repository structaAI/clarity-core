...
-------------------------------
The DDIMScheduler was rebuilding alphas_cumprod from hardcoded beta values
(beta_start=1e-4, beta_end=0.02). If GaussianDiffusion uses different values
or a different schedule type, every DDIM step uses the wrong noise level,
causing the denoising direction to be wrong at every step — producing pure
noise regardless of step count.

Fix: import GaussianDiffusion and read its alphas_cumprod buffer directly.
The DDIM scheduler now shares the EXACT same schedule the model was trained on.
...
Optional:
  NUM_INFERENCE_STEPS   — default 200
  NUM_SAMPLES           — runs to average (default 4)
  DDIM_ETA              — 0.0=deterministic, 0.2=slight stochasticity (default 0.0)
...
class DDIMScheduler:
    """
    DDIM sampler that reuses alphas_cumprod from the training
    ...
    def __init__(
        ...
        # Use the EXACT same alphas_cumprod as training — not rebuilt from betas
        ...
        log.info(f"  alphas_cumprod at first/last timestep: "
...
def main() -> None:
    ...
    # Build GaussianDiffusion — same object training used
    diffusion = GaussianDiffusion(
    ...
    # Build DDIM from the SAME diffusion buffers
    ...
    log.info(f"Loading VAE: {VAE_PATH}")
    ...
    # Multi-sample DDIM
    log.info(f"\nRunning {NUM_SAMPLES} x DDIM-{NUM_STEPS}...")
    ...
    log.info(f"  Time: {elapsed:.1f}s  ({elapsed/NUM_SAMPLES:.1f}s/sample)")
    ...
    print(f"  Time        : {elapsed:.1f}s  ({elapsed/NUM_SAMPLES:.1f}s/sample)")
    print()
