...
---------------------
BUG FIX — test_invalid_schedule_raises:
    Was passing schedule="linear" (a VALID value) to GaussianDiffusion, so
    pytest.raises(ValueError) would never trigger and the test would always
    fail.  Fixed to pass an actually invalid schedule string.
"""
...
class TestGaussianDiffusion:
    ...
    def test_invalid_schedule_raises(self):
        # FIX: was GaussianDiffusion(schedule="linear") — "linear" IS valid and
        # would never raise ValueError, causing the test to always fail.
        with pytest.raises(ValueError, match="Unsupported schedule"):
            GaussianDiffusion(schedule="quadratic_bogus") # type: ignore
...
class TestInferencePipeline:
    ...
    def _make_pipeline(self):
        ...
        pipeline = InferencePipeline(denoiser=denoiser, diffusion=diff, config=cfg)
        os.unlink(cfg_path)
        return pipeline
    ...
    def test_super_resolve_from_latent(self):
        """Test SR mode without VAE using lr_is_latent=True (no monkey-patching needed)."""
        pipeline = self._make_pipeline()
        x_lr_latent = torch.randn(1, 4, 8, 8)
        out = pipeline.super_resolve(x_lr_latent, lr_is_latent=True)  # type: ignore
        assert out.shape == (1, 4, 8, 8)
