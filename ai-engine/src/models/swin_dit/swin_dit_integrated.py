...
from src.models.bridge.auth_bridge import AuthBridge # Assuming AuthBridge is in auth_bridge.py
...
class SwinDiT(nn.Module):
    def __init__(self, config) -> None:
        ...
        # The Conditioning Bridge for Integrated Restoration
        self.auth_bridge = AuthBridge(
            input_dim=c.embed_dim,
            output_dim=c.embed_dim,
            cond_dim=c.embed_dim
        )
    ...
    def forward(self, x, t=None, clip_embeddings=None, degradation_type=None, severity=None, precomputed_cond=None):
        ...
        # 2. Conditioning: Use fused precomputed context if provided
        ...
        # 3. Swin blocks with Gated Residual Conditioning
        for block in self.blocks:
            # The AuthBridge refines tokens 'x' based on the integrated condition
            x = self.auth_bridge(x, t_feature)
            x = block(x, t_feature, H, W)

        # 4. Fold back to Image Space
        x = self.final_layer(x)
        B, N, Cpatch = x.shape
        in_ch = self.patch_embedding.projection.in_channels
        p = int((Cpatch // in_ch) ** 0.5)
        
        return (x.view(B, H, W, p, p, in_ch)
                .permute(0, 5, 1, 3, 2, 4)
                .contiguous()
                .view(B, in_ch, H * p, W * p))